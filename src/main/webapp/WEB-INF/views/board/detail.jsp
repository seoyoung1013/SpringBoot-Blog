...
<section class="probootstrap-cover overflow-hidden relative" style="background-image: url('/image/bg_1.jpg');" data-stellar-background-ratio="0.5" id="section-home">
	...
	<div class="container">
		<div class="row align-items-center text-center">
			<div class="col-md">
				<h2 class="heading mb-2 display-4 font-light probootstrap-animate">여행 동행자 모집 게시판</h2>
			</div>
		</div>
	</div>
</section>
...
<section class="probootstrap_section">
	<div class="container">
		...
		<button id="btn-show-profile-Info" class="btn btn-info" data-user-id="${board.user.id}">작성자 정보</button>
		<c:choose>
		...
		<br /> <br />
		<div>
			글 번호 : <span id="id"> <i>${board.id} </i>
			</span> 작성자 : <span> <i>${board.user.username} </i>
			</span> 동행자 매칭 여부 : <span id="matchingStatus"><i>${board.travelWith == 'yes' ? '매칭 성공' : '매칭 전'}</i></span>
		</div>

		<br />
		<div>
			<h3 style="display: inline-block;">${board.title}</h3>
		</div>
